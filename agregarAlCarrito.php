...
$sentencia = $base_de_datos->prepare("SELECT * FROM tbl_libro WHERE codigo = ? LIMIT 1;");
...
session_start();
# Buscar libro dentro del cartito
$indice = false;
...
if ($indice === false) {
    $libro->cantidad = 1;
    $libro->total = $libro->precio;
    array_push($_SESSION["carrito"], $libro);
} else {
    # Si ya existe, se agrega la cantidad
    # Pero espera, tal vez ya no haya
    $cantidadExistente = $_SESSION["carrito"][$indice]->cantidad;
    # si al sumarle uno supera lo que existe, no se agrega
    if ($cantidadExistente + 1 > $libro->stock) {
        header("Location: ./comprar.php?status=5");
        exit;
    }
